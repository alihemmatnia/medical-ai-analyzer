...
import { AlertCircle, Activity, Stethoscope, ArrowLeft, MessageSquare, AlertTriangle } from 'lucide-react';
...
export default function Analysis() {
    ...
    return (
      <div className="flex h-[80vh] items-center justify-center flex-col gap-6">
        ...
        <div className="text-center space-y-1.5">
          <div className="text-slate-200 font-bold text-sm uppercase tracking-wider animate-pulse">{t('analysis.loadingTitle')}</div>
          <div className="text-slate-500 text-xs">{t('analysis.loadingSub')}</div>
        </div>
      </div>
    );
  }
  ...
  if (!report) {
    return (
      <div className="max-w-2xl mx-auto mt-12 text-center space-y-4">
        ...
        <h2 className="text-xl font-bold text-white">{t('analysis.reportNotFound')}</h2>
        <p className="text-slate-400 text-sm">{t('analysis.reportNotFoundSub')}</p>
  ...
  const currentUrgencyVal = analysis?.urgency || 'Normal';
  const localizedUrgencyText = t(`analysis.urgency_${currentUrgencyVal.toLowerCase()}`, { defaultValue: currentUrgencyVal });
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-800/60 pb-6">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-black text-white">{t('analysis.title')}</h1>
          <p className="text-slate-400 text-xs font-medium">{t('common.record', { filename: report.filename })}</p>
        </div>
        
        <div className={`px-4.5 py-2 rounded-full border text-xs font-extrabold tracking-wider uppercase self-start sm:self-auto ${urgencyColors[currentUrgencyVal] || urgencyColors["Normal"]}`}>
          {t('common.urgency', { urgency: localizedUrgencyText })}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-2xl border border-slate-800/80 hover:border-brand-indigo/35 transition-all duration-300 relative group overflow-hidden">
          ...
          <div className="flex items-center gap-2.5 mb-4 relative z-10">
            ...
            <h2 className="text-base font-bold text-white">{t('analysis.execSummary')}</h2>
          </div>
        ...
        <div className="glass-card p-6 rounded-2xl border border-slate-800/80 hover:border-brand-rose/25 transition-all duration-300 relative group overflow-hidden">
          ...
          <div className="flex items-center gap-2.5 mb-4 relative z-10">
            ...
            <h2 className="text-base font-bold text-white">{t('analysis.abnormalFindings')}</h2>
          </div>
        ...
        <div className="glass-card rounded-2xl border border-slate-800/80 md:col-span-2 overflow-hidden hover:border-slate-800 transition-all duration-300">
          <div className="p-6 border-b border-slate-800/60 bg-slate-900/40 flex items-center gap-2.5">
            ...
            <h2 className="text-base font-bold text-white">{t('analysis.labValuesTitle')}</h2>
          </div>
            ...
            <table className="w-full text-start border-collapse">
              <thead>
                <tr className="bg-slate-950/40 border-b border-slate-800/60">
                  <th className="p-4 text-xs font-bold uppercase tracking-wider text-slate-400 text-start">{t('analysis.testParameter')}</th>
                  <th className="p-4 text-xs font-bold uppercase tracking-wider text-slate-400 text-start">{t('analysis.resultValue')}</th>
                  <th className="p-4 text-xs font-bold uppercase tracking-wider text-slate-400 text-start">{t('analysis.refRange')}</th>
                  <th className="p-4 text-xs font-bold uppercase tracking-wider text-slate-400 text-start">{t('analysis.unit')}</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={lab.id} className={`hover:bg-slate-900/30 transition-colors ${isOut ? 'bg-brand-rose/5' : ''}`}>
                      <td className="p-4 text-sm font-semibold text-slate-200 flex items-center gap-2">
                        {lab.test_name}
                        {isOut && <span className="px-2 py-0.5 text-[10px] font-extrabold bg-brand-rose/15 text-brand-rose border border-brand-rose/25 rounded-md uppercase tracking-wide">{t('common.alert')}</span>}
                      </td>
      ...
      <div className="mt-8 flex justify-center">
        ...
        >
          <MessageSquare size={18} /> {t('analysis.discussBtn')}
        </Link>
      </div>
    </div>
  );
