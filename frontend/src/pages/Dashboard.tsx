...
export default function Dashboard() {
  ...
  // Synonym mappings for trend data normalization
  const glucoseSynonyms = ['glucose', 'گلوکز', 'الجلوكوز', 'גלוקוז', 'glukose', 'glucosa'];
  const cholesterolSynonyms = ['cholesterol', 'کلسترول', 'الكوليسترول', 'כולסטרול', 'cholestérol', 'cholesterin', 'colesterol'];

  // Current language keys
  const glucoseKey = language === 'fa' ? 'گلوکز' : language === 'ar' ? 'الجلوكوز' : language === 'he' ? 'גלוקוז' : language === 'fr' ? 'Glucose' : language === 'de' ? 'Glukose' : language === 'es' ? 'Glucosa' : 'Glucose';
  const cholesterolKey = language === 'fa' ? 'کلسترول' : language === 'ar' ? 'الكوليسترول' : language === 'he' ? 'כולسترول' : language === 'fr' ? 'Cholestérol' : language === 'de' ? 'Cholesterin' : language === 'es' ? 'Colesterol' : 'Cholesterol';

  // Normalize backend trend metrics keys to match the user's selected language
  const normalizedTrends = data?.trends?.map((point: any) => {
    const newPoint = { ...point };
    
    let glucoseVal: number | undefined;
    for (const key of Object.keys(point)) {
      if (glucoseSynonyms.includes(key.toLowerCase())) {
        glucoseVal = point[key];
      }
    }
    
    let cholesterolVal: number | undefined;
    for (const key of Object.keys(point)) {
      if (cholesterolSynonyms.includes(key.toLowerCase())) {
        cholesterolVal = point[key];
      }
    }
    
    if (glucoseVal !== undefined) {
      newPoint[glucoseKey] = glucoseVal;
    }
    if (cholesterolVal !== undefined) {
      newPoint[cholesterolKey] = cholesterolVal;
    }
    return newPoint;
  }) || [];

  const hasTrends = normalizedTrends.length > 0;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800/60 pb-6">
        ...
        >
          <Zap size={16} /> {t('dashboard.uploadBtn')}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 rounded-2xl border border-slate-800/80 hover:border-brand-indigo/35 transition-all duration-300 relative group overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">{t('dashboard.healthScore')}</div>
        ...
        <div className="glass-card p-6 rounded-2xl border border-slate-800/80 hover:border-brand-rose/25 transition-all duration-300 relative group overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">{t('dashboard.urgencyStatus')}</div>
              <div className="text-xl font-bold text-white mt-1.5 flex items-center gap-1.5">
                ...
                {data?.ai_findings?.urgency ? t(`analysis.urgency_${data.ai_findings.urgency.toLowerCase()}`, { defaultValue: data.ai_findings.urgency }) : t('analysis.urgency_normal', { defaultValue: 'Normal' })}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-6 rounded-2xl border border-slate-800/80 hover:border-brand-cyan/35 transition-all duration-300 relative group overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <div className="text-slate-400 text-xs font-semibold uppercase tracking-wider">{t('dashboard.filesAnalyzed')}</div>
              <div className="text-3xl font-black text-white mt-1">
                {data?.recent_reports?.length || 0} <span className="text-slate-500 text-xs font-normal lowercase">{t('dashboard.records')}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="glass-card p-6 rounded-2xl border border-slate-800/80 hover:border-slate-800 transition-all duration-300 relative">
        <h2 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
          <Activity size={18} className="text-brand-cyan" /> {t('dashboard.aiSummaryTitle')}
        </h2>
        <p className="text-slate-300 text-sm leading-relaxed max-w-4xl">
          {data?.ai_findings?.summary || t('dashboard.aiSummaryEmpty')}
        </p>
      </div>
      ...
      {hasTrends && (
        <div className="glass-card p-6 rounded-2xl border border-slate-800/80 hover:border-slate-800 transition-all duration-300">
          <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
            <Activity size={18} className="text-brand-indigo" /> {t('dashboard.trendsTitle')}
          </h2>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={normalizedTrends} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
                ...
                <Area type="monotone" dataKey={glucoseKey} stroke="#06b6d4" strokeWidth={2.5} fillOpacity={1} fill="url(#colorGlucose)" />
                <Area type="monotone" dataKey={cholesterolKey} stroke="#6366f1" strokeWidth={2.5} fillOpacity={1} fill="url(#colorCholesterol)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="glass-card rounded-2xl border border-slate-800/80 overflow-hidden hover:border-slate-800 transition-all">
        <div className="p-6 border-b border-slate-800/60 flex justify-between items-center bg-slate-900/40">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <Calendar size={18} className="text-brand-cyan" /> {t('dashboard.recentUploads')}
          </h2>
        </div>
            ...
            <div key={report.id} className="p-4 hover:bg-slate-900/20 transition-colors flex flex-col sm:flex-row justify-between sm:items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">{new Date(report.uploaded_at).toLocaleDateString(language, { year: 'numeric', month: 'short', day: 'numeric' })}</div>
                </div>
              </div>
              <Link 
              ...
              >
                {t('dashboard.viewAnalytics')} <ArrowRight size={14} className="group-hover:translate-x-0.5 transition-transform" />
              </Link>
            </div>
          ))}
          {(!data?.recent_reports || data.recent_reports.length === 0) && (
            <div className="text-slate-500 text-sm py-10 text-center font-medium">
              {t('dashboard.recentUploadsEmpty')}
            </div>
          )}
        </div>
      </div>
